...
void PointerExample()
{
	...
	// 변수의 이름으로 값을 가져올 수 있고, 주소로부터 값을 가져올 수도 있습니다.

	// 주소 형태로 가져오는 방식 (참조 방식)	- 프로그램이 종료되거나, 직접 메모리를 해제하기 전까지 계속 남습니다.	- 힙(Heep) 영역
	// 변수의 이름으로 가져오는 방식(값 방식)	- () 코드 영역이 끝나는 시점에 메모리가 해제됩니다.					- 스택(stack) 영역

	// 모든 코드를 항상 저장하면 안될까
	// 게임 점수를 일정 값만큼 증가시키는 코드를 작성하고 싶다
	// 저장되어야 할 데이터 : 게임 점수
	// 한번 사용하고 지우면 좋을 데이터 : 게임 점수를 증가하고 싶은 가중치

	// 모든 코드를 한 번만 사용하고 저장을 안하면?
	// GameScore 포인터로 변경해서 저장하는 기능을 구현할 수 없게 된다.

	// call by value, call by reference 구분해서 사용해야하는 이유
